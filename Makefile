...
init:
	mkdir -p files
	sed 's/\$$address/$(address)/;s/\$$email/$(email)/g;s/\$$phone_number/$(phone_number)/' resume.md > files/resume.md
download-css: init
...
generate-pdf: generate-html
	wkhtmltopdf --encoding utf-8 files/resume.html files/resume.pdf
